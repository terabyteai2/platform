...
import {
  clusterImageFromRecord,
  clusterImagesByIds,
  clusterImageToData,
  isClusterImageSchemaMissing,
  selectClusterImage,
  type ClusterImage,
...
export async function POST(
  ...
  if (parsed.data.mode === "manual") {
    ...
    let updated;
    try {
      updated = await db.cluster.update({
        where: { id },
        data: clusterImageToData(image),
      });
    } catch (err) {
      if (isClusterImageSchemaMissing(err)) {
        return Response.json(
          { error: "Cluster image columns are not in the database yet. Run npm run db:push." },
          { status: 409 }
        );
      }
      throw err;
    ...
    return Response.json({ image: clusterImageFromRecord(updated) });
  }

  const image = await selectClusterImage(cluster);
  if (!image) {
    const imageMap = await clusterImagesByIds([id]);
    return Response.json({
